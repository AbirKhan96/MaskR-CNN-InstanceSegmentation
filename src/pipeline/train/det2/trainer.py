...
class Det2Trainer:
    ...
    # ======================================================================================================
    def proc_json_files(self, all_jsons_dir, write_dir, to_shape, ext, pipeline=['write_original_image', 'write_reshaped_b64json'], ALLOWED_CLASSES = []):
    ...
    # beg: to coco format
    # ======================================================================================================
    def labeme_to_coco(self, base_dir, ALLOWED_CLASSES ,list_of_dirs=['train', 'test', 'holdout']):

        for dirname in list_of_dirs:

            dir_path = base_dir + '/' + dirname
            json_files = get_files_of(ext='json', at=dir_path)
            labelme2coco(json_files, ALLOWED_CLASSES,  save_json_name=base_dir +
                         dirname + '.json')
    # ======================================================================================================
    # end: to coco format
    # ======================================================================================================
    ...
    def prepare_data(self, dta_prp_cfg):
        ...
        if dta_prp_cfg.proc_json_files is True:
            self.proc_json_files(
                ...
                ALLOWED_CLASSES=self.dta_cfg.ALLOWED_CLASSES)
        ...
        if dta_prp_cfg.labelme2coco is True:
            print (self.dta_cfg.ALLOWED_CLASSES)
            self.labeme_to_coco(
